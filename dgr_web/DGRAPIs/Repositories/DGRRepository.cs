...
bd_remarks, action_taken
 FROM uploading_file_breakdown_solar t1 left join site_master_solar t2 on t2.site_master_solar_id=t1.site_id ";
        ...
        internal async Task<int> InsertSolarSoilingLoss(List<InsertSolarSoilingLoss> set)
        {
            ...
            string qry = " insert into uploading_file_soiling_loss (site_name, site_id, month, month_no, five_days, ten_days, fifteen_days, fifteen_days_original, rainy_days, sandstorm_days, total_rain, manual_scada, is_manual_scada, reason) values";
            string insertValues = "";
            string deleteValues = "";
            string month = "(";
            string id = "(";
            ...
            foreach (var unit in set)
            {
                insertValues += "('" + unit.site_name + "','" + unit.site_id + "','" + unit.month + "','" + unit.month_no + "','" + unit.five_days + "','" + unit.ten_days + "','" + unit.fifteen_days + "','" + unit.fifteen_days_original + "','" + unit.rainy_days + "','" + unit.sandstorm_days + "','" + unit.total_rain + "','" + unit.manual_or_SCADA + "','" + unit.isManual_or_SCADA + "','" + unit.reason + "'),";
        ...
        internal async Task<int> DeleteRecordsAfterFailure(int batchId, int siteType)
        {
            ...
            if (siteType == 0)
            {
                ...
                if (solarFileGeneration && solarFileBreakdown && solarPyranometerOne && solarPyranometerFifteen && importBatch)
                {
                    return 1;
                }
                else
            ...
            if (siteType == 1)
            {
                ...
                if (windFileBreakdown && windFileGeneration)
                {
                ...
                if (windFileBreakdown && windFileGeneration && importBatch)
                {
                    return 1;
                }
                else
...
daily_target_kpi_solar_id desc limit 1) as tarIR from daily_gen_summary_solar t1 where t1.daily_gen_summary_solar_id in(" + ids + ") ";
        ...
        private async Task<bool> CalculateAndUpdateKPIs(int site_id, string fromDate, string sWTG_Name, TimeSpan Final_USMH_Time, TimeSpan Final_SMH_Time, TimeSpan Final_IGBD_Time, TimeSpan Final_EGBD_Time, TimeSpan Final_OthersHour_Time, TimeSpan Final_LoadShedding_Time, string MA_Actual_Formula, string MA_Contractual_Formula, string IGA_Formula, string EGA_Formula)
        {
            ...
            try
            {
                ...
                //double dEGA_C = Math.Round(GetCalculatedValue(Final_USMH, Final_SMH, Final_IGBD, Final_EGBD, Final_OthersHour, 0, EGA_Formula), 6);



                string qryUpdate = "UPDATE `uploading_file_generation` set ma_actual = " + dMA_ACT + ", ma_contractual = " + dMA_CON + ", iga = " + dIGA + ", ega = " + dEGA + ", ega_b = " + dEGA_B;
            ...
            try
            {
                ...
                foreach (SolarLocationMaster_Calc SolarDevice in _SolarLocationMaster_Calc)
                {
                    ...
                    if (sLastICR_INV != sCurrentICR_INV || iBreakdownCount == _SolarLocationMaster_Calc.Count)
                    {
                        ...
                        string updateqry = "update uploading_file_generation_solar set ghi = " + avg_GHI + ", poa= " + avg_POA + ", expected_kwh=" + (FinalCapcity * avg_POA) +
                            ", ma=100, iga=100, ega=100, ega_b=100, ega_c =100, inv_pr=inv_act*100/" + (FinalCapcity * avg_POA) + ", plant_pr=plant_act*100/" + (FinalCapcity * avg_POA) +
